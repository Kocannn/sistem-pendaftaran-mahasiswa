...
$query = "SELECT skor_ujian FROM mahasiswa WHERE nisn = ?";
...
// If student has a score, they've completed the exam, so redirect them
if (!empty($student['skor_ujian'])) {
    $_SESSION['exam_message'] = "Anda sudah menyelesaikan ujian. Tidak dapat mengikuti ujian lagi.";
...
<body class="bg-gray-50 font-poppins min-h-screen">
  ...
  <!-- Submit Confirmation Modal - Improved centering and mobile display -->
  <div id="submitModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 my-10">
  ...
  <!-- Logout Confirmation Modal - Improved centering and mobile display -->
  <div id="logoutModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 my-10">
      <div class="text-center p-6">
  ...
  <!-- Result Modal - Improved centering and mobile display -->
  <div id="resultModal" class="hidden fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 overflow-y-auto">
    <div class="bg-white rounded-xl shadow-2xl max-w-md w-full mx-4 my-10">
      <div class="text-center p-6">
        ...
        <h3 class="text-xl font-bold text-gray-900 mb-2">Ujian Selesai!</h3>
        <p class="text-gray-600 mb-4">Terima kasih telah menyelesaikan ujian.</p>
        
        <div class="bg-gray-50 rounded-lg p-4 mb-6">
          <div class="text-center mb-2">
            <p class="text-sm text-gray-500">Skor Anda:</p>
            <p id="finalScore" class="text-3xl font-bold text-academic-blue-600">0.0</p>
          </div>
          <div id="resultMessage" class="text-sm text-gray-600 text-center">
            Hasil sedang diproses...
          </div>
        </div>
        
        <button id="resultBackBtn" class="w-full py-3 px-4 bg-academic-blue-600 text-white rounded-lg font-medium hover:bg-academic-blue-700 transition-colors duration-200">
  ...
  <script>
    ...
    // Correct answers for automatic grading (A=0, B=1, C=2, D=3)
    const correctAnswers = [1, 0, 2, 1, 2, 1, 2, 1, 1, 2];
    ...
    function calculateScore() {
      ...
      for (let i = 0; i < examData.length; i++) {
        ...
        const answerIndex = answers[i] ? answers[i].charCodeAt(0) - 65 : -1;
      ...
      return (correctCount / examData.length) * 100;
    }
    ...
    function submitExam() {
      ...
      // Display loading state in result modal
      document.getElementById('finalScore').innerText = score.toFixed(1);
      document.getElementById('resultMessage').innerText = 'Sedang memproses hasil...';
      document.getElementById('resultBackBtn').disabled = true;
      ...
      fetch('/sistem-informasi-pendaftaran/service/mahasiswa/update_exam_score.php', {
        method: 'POST',
        body: formData
      })
      ...
      .then(data => {
        if (data.success) {
          // Show success message with admission status
          let statusMessage = '';
          if (data.status_kelulusan === 'lulus') {
            statusMessage = 'Selamat! Anda ' + (data.admission_status.includes('Pilihan 1') ? 
              'diterima pada pilihan pertama.' : 'diterima pada pilihan kedua.');
          } else {
            statusMessage = 'Mohon maaf, Anda belum memenuhi kriteria kelulusan.';
          }
          
          document.getElementById('resultMessage').innerHTML = 
            `<span class="font-medium">${statusMessage}</span><br>` +
            `<span class="text-xs">Status: ${data.admission_status}</span>`;
        } else {
          // Show error message
          document.getElementById('resultMessage').innerHTML = 
            '<span class="text-red-600">Gagal menyimpan hasil. Silakan hubungi administrator.</span>';
        }
        
        // Enable back button
        document.getElementById('resultBackBtn').disabled = false;
      })
      .catch(error => {
        console.error('Error:', error);
        document.getElementById('resultMessage').innerHTML = 
          '<span class="text-red-600">Terjadi kesalahan. Hasil ujian mungkin tidak tersimpan.</span>';
        document.getElementById('resultBackBtn').disabled = false;
      });
    }

    // Event listener for result back button
    document.getElementById('resultBackBtn').addEventListener('click', function() {
      window.location.href = '/sistem-informasi-pendaftaran/mahasiswa';
    });
    ...
    // Add function to handle logout modal display
    ...
    function confirmLogout() {
      // Redirect to dashboard or logout endpoint
      window.location.href = '/sistem-informasi-pendaftaran/mahasiswa';
    }
    
    // Add function to handle submit modal display
    function showSubmitModal() {
      const answeredCount = Object.keys(answers).length;
      const unansweredCount = examData.length - answeredCount;
    ...
    document.querySelectorAll('#submitModal, #logoutModal, #resultModal').forEach(modal => {
    ...
    // Add keyboard support - escape key to close modals
